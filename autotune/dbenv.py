...
class PostgresEnv(DBEnv):
    def __init__(self,
        ...
        self.best_result = './autotune_best.res'
        self.knobs_config = knobs_config
        self.knobs_detail = init_knobs(knobs_config)
        self.default_knobs = get_default_knobs()
        self.rds_mode = rds_mode
        self.oltpbench_config_xml = oltpbench_config_xml
        self.step_count = 0
    ...
    def reset_internal_state(self):
        db_conn = PostgresConnector(host=self.host,
                                 port=self.port,
                                 user=self.user,
                                 passwd=self.passwd,
                                 name=self.dbname)
        ...
        sql1 = 'SELECT pg_stat_statements_reset();'
        db_conn.execute(sql1)
        sql1 = 'SELECT pg_stat_reset()'
        db_conn.execute(sql1)
    ...
    def get_internal_metrics(self, internal_metrics):
        ...
        This func uses a SQL statement to lookup system table: 
    ...
    def _gen_config_file(self, knobs):
        ...
        for key in knobs.keys():
            if not key in self.knobs_detail.keys():
                knobs_not_in_mycnf.append(key)
                continue
            cnf_parser.set(key, knobs[key])
        cnf_parser.replace()
        logger.info('generated config file done')
        return knobs_not_in_mycnf
    ...
    def _start_psqld(self):
        ...
        while True:
            try:
                dbc = PostgresConnector(host=self.host,
                                         port=self.port,
                                         user=self.user,
                                         passwd=self.passwd,
                                         name=self.dbname)

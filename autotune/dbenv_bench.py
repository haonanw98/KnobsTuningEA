...
class PostgreSQLEnv(DBEnv):
    ...
    def generate_time(self):
        ...
        if self.workload['name'] == 'sysbench' or self.workload['name'] == 'oltpbench' or self.workload['name'] == 'benchbase':
            BENCHMARK_RUNNING_TIME = 120
            BENCHMARK_WARMING_TIME = 30
            TIMEOUT = BENCHMARK_RUNNING_TIME + BENCHMARK_WARMING_TIME + 15
            RESTART_FREQUENCY = 200
    ...
    def get_db_size(self):
        db_conn = PostgresConnectorz(host=self.host,
                                 port=self.port,
                                 user=self.user,
                                 passwd=self.passwd,
                                 name=self.dbname,
